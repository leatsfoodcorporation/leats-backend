...
// Generate random password — uses only copy-paste-safe characters
// Avoids special chars that get corrupted in HTML emails (& ^ < > " ')
// and ambiguous chars (0/O, 1/l/I)
function generatePassword(length = 12) {
  const lower = "abcdefghjkmnpqrstuvwxyz";
  const upper = "ABCDEFGHJKMNPQRSTUVWXYZ";
  const digits = "23456789";
  const safe = "@#!";
  const all = lower + upper + digits + safe;
  ...
  password += safe.charAt(Math.floor(Math.random() * safe.length));
...
async function sendRegistrationSubmittedEmail(partner) {
  try {
    const emailTemplate = await getRegistrationSubmittedEmailTemplate({
      name: partner.name,
      email: partner.email,
    });
    ...
    if (result.success) {
      console.log(`✅ Registration confirmation email sent to: ${partner.email}`);
    } else {
      console.error(`❌ Failed to send registration confirmation email: ${result.message}`);
    }
  } catch (error) {
...
const createDeliveryPartner = async (req, res) => {
  ...
  try {
    const {
      ...
      applicationStatus,
      password: adminProvidedPassword, // Admin can provide custom password
    } = req.body;
    ...
    if (existingEmail) {
      // Allow re-registration if previous application was rejected
      if (existingEmail.applicationStatus === "rejected") {
        await prisma.deliveryPartner.delete({ where: { id: existingEmail.id } });
        console.log(`🔄 Deleted rejected partner record for re-registration: ${email}`);
      } else {
        return res.status(400).json({
          success: false,
    ...
    if (existingPhone) {
      if (existingPhone.applicationStatus === "rejected") {
        await prisma.deliveryPartner.delete({ where: { id: existingPhone.id } });
        console.log(`🔄 Deleted rejected partner record for re-registration: ${phone}`);
      } else {
        return res.status(400).json({
          success: false,
    ...
    if (existingVehicle) {
      if (existingVehicle.applicationStatus === "rejected") {
        await prisma.deliveryPartner.delete({ where: { id: existingVehicle.id } });
      } else {
        return res.status(400).json({
          success: false,
    ...
    if (existingLicense) {
      if (existingLicense.applicationStatus === "rejected") {
        await prisma.deliveryPartner.delete({ where: { id: existingLicense.id } });
      } else {
        return res.status(400).json({
          success: false,
    ...
    // Generate a temporary unique partnerId for non-approved registrations
    // MongoDB unique constraint does not allow multiple null values
    const tempPartnerId = `PENDING-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
...
const getDeliveryPartnerById = async (req, res) => {
  try {
    ...
    const completedOrdersCount = await prisma.onlineOrder.count({
      where: {
    ...
    const recentCompletedOrders = await prisma.onlineOrder.findMany({
      where: {
        deliveryPartnerId: id,
        orderStatus: 'delivered',
      },
