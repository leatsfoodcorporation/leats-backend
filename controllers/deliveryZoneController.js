...
const createDeliveryZone = async (req, res) => {
  try {
    const { country, state, city, pincodes, isActive, isAllPincodes } = req.body;
    ...
    if (!isAllPincodes && (!pincodes || !Array.isArray(pincodes) || pincodes.length === 0)) {
      return res.status(400).json({
        success: false,
    ...
    if (existing) {
      return res.status(409).json({
        success: false,
        message: `Delivery zone for ${city}, ${state} already exists`,
      });
    }
    ...
    const zone = await prisma.deliveryZone.create({
      data: {
        ...
        isAllPincodes: isAllPincodes || false,
      },
    });
