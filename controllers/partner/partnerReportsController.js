...
const getPartnerReports = async (req, res) => {
  try {
    ...
    // Calculate peak hours based on delivery completion time
    const hourCounts = new Array(24).fill(0);
    deliveredOrders.forEach(order => {
      // Use deliveredAt (actual delivery time), fall back to createdAt
    ...
    if (maxCount > 0) {
      ...
      const endHour = (peakHour + 1).toString().padStart(2, '0');
      peakHours = `${startHour}:00 - ${endHour}:00`;
    }

    // Calculate average orders per day (only counting days that had orders)
    const activeDays = new Set();
    orders.forEach(order => {
      const d = new Date(order.createdAt);
      activeDays.add(`${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`);
    });
    const avgOrdersPerDay = activeDays.size > 0
      ? parseFloat((totalOrders / activeDays.size).toFixed(1))
      : 0;
