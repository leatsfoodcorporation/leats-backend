...
async function getKPICards(startDate, endDate, prevStartDate, prevEndDate) {
  try {
    ...
    const [
      currentPosOrders,
      currentOnlineOrders,
      currentDeliveredOrders, // NEW: Delivered orders filtered by deliveredAt
      prevPosOrders,
      prevOnlineOrders,
      warehouses,
      customers
      ...
      // Current Online Orders (for revenue calculation)
      ...
      // Current Delivered Orders (filtered by deliveredAt date)
      prisma.onlineOrder.findMany({
        where: {
    ...
    // Calculate on-time delivery percentage from OnlineOrders
    // Count delivered orders by deliveredAt date (not createdAt)
    const deliveredOnlineOrdersByDeliveryDate = currentDeliveredOrders.length;
    const completedPosOrdersByDate = currentPosOrders.filter(o => o.orderStatus === 'completed').length;
    const totalDeliveredOrders = deliveredOnlineOrdersByDeliveryDate + completedPosOrdersByDate;
    
    // Calculate on-time delivery for online orders
    // Consider on-time if delivered within expected timeframe (e.g., within 3 days of order creation)
    let onTimeDelivery = 0;
    if (totalDeliveredOrders > 0) {
      const onTimeOnlineOrders = currentDeliveredOrders.filter(order => {
        if (!order.deliveredAt || !order.createdAt) return false;
        const orderDate = new Date(order.createdAt);
        const deliveredDate = new Date(order.deliveredAt);
        const deliveryTime = (deliveredDate - orderDate) / (1000 * 60 * 60 * 24); // days
        return deliveryTime <= 3; // Consider on-time if delivered within 3 days
      }).length;
      
      // POS orders are considered instant delivery (always on-time)
      const onTimeCount = onTimeOnlineOrders + completedPosOrdersByDate;
      onTimeDelivery = (onTimeCount / totalDeliveredOrders) * 100;
    }
    ...
    console.log(`📊 KPI Cards: Orders=${totalOrders}, Revenue=₹${totalRevenue.toFixed(2)}, AOV=₹${avgOrderValue.toFixed(2)}, Cancelled=${cancelledOrders}, Delivered=${totalDeliveredOrders}, On-Time=${onTimeDelivery.toFixed(1)}%`);
    ...
    return {
      ...
      deliveredOrders: totalDeliveredOrders, // NEW: Total delivered orders count
